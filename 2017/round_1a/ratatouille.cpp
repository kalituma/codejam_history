 ...
 * the ultimate recipe for ratatouille, the famous French dish! You know which ingredients to use,
 * and how many grams of each one to use, in order to make one serving of ratatouille.
 ...
 * Each package contains some amount of one ingredient; different packages may have different amounts even if they contain the same ingredient.
 ...
 * Suppose that you have a 900 g package of tomato and a 660 g package of onion. You could form these into a kit that makes two servings of ratatouille.
 * To make two servings, 1000 g of tomato and 600 g of onion are required. Since the 900 g of tomato you have is within [90, 110]% of the 1000 g of tomato required,
 ...
 * Three servings would take 1500 g of tomato and 900 g of onion, and the amount of onion is not within the [90, 110]% range. No other integer amount of servings works, either.
 *
 * You want to share your recipe with as many customers as possible,
 * so you want to produce the maximum number of valid kits. (Of course, each package can be used in at most one kit.)
 ...
 * In sample case #3, you can form a kit out of the 450 g package of the first ingredient and the 1100 g package of the second ingredient,
 * and say that the kit makes 10 servings of ratatouille. That number of servings requires 500 g of the first ingredient;
 * you have 450 g, which is 90% of 500 and within the allowed limit. It requires 1000 g of the second ingredient;
 * you have 1100 g, which is 110% of 1000 and within the allowed limit.
 *
 * Once you form this kit, however, you cannot form the remaining packages into a kit.
 * 449 g of the first ingredient and 1101 g of the second ingredient would not be able to form 10 (or any other number of) servings.
 * In fact, the (450 g, 1100 g) kit is the only kit that can be formed from these packages.
 *
 * In sample case #4, no kits can be formed. Note that the recipe requires particular amounts of particular ingredients in the given order;
 * the ingredients are not interchangeable. This is fine French cuisine, after all!
 *
 * In sample case #5, the recipe has only one ingredient — how elegantly simple! A single serving cannot use more than 11 g,
 * and two servings cannot use fewer than 18 g. It is possible to form three kits: two with an 11 g package, and one with an 18 g package.
 *
 * In sample case #6, you can form three valid kits: (700 g, 800 g, 900 g),
 * which makes 10 servings, and (1500 g, 1600 g, 1700 g) and (1260 g, 1440 g, 1620 g), each of which makes 20 servings.
...
/*
 * code from https://github.com/ruippeixotog/google-code-jam-2017/blob/master/round1a/ratatouille.cpp
 */
#include <algorithm>
#include <cstdio>
#include <cstring>
