 ...
 * This system serves N different cities. In each city, there is one horse (as in the expression "one-horse town");
 * each horse travels at a certain constant speed and has a maximum total distance it can travel before it becomes too tired to continue.
 *
 ...
 *
 * The routes between cities were established via complicated negotiations between company owners, lawmakers, union delegates, and cousin Pete.
 * That means that the distances between cities do not necessarily follow common sense:
 * for instance, they do not necessarily comply with the triangle inequality,
 * and the distance from city A to city B might be different from the distance from city B to city A!
 *
 ...
 * The j-th integer on the i-th of these lines, Dij, is -1 if there is no direct route from the i-th to the j-th city, and the length of that route in kilometers otherwise.
...
/*
 * For each test case, output one line containing Case #x: y1 y2 ... yQ, where x is the test case number (starting from 1) and
 * yk is the minimum time, in hours, to deliver a letter from city Uk to city Vk.
 *
 ...
 *
 * In Case #1 there are two options:
 * use the horse in city 1 for the entire trip, or change horses in city 2.
 * Both horses have enough endurance, so both options are viable. Since the horse in city 2 is faster, it is better to change, for a total time of 1/3 + 1/4.
 *
 ...
 * so you will be forced to change again in city 3. If you keep your horse,
 * you will have the option to change horses (or not) in city 3. So, the three options, with their total times, are:
 *
 ...
 *
 * For the second delivery (city 3 to city 2) you have no choice but to first go to city 4 which takes time 10/5.
 *
 * Your relatively fast horse does not have enough endurance to get anywhere else, so you need to grab the horse in city 4.
 * You could use it to get directly to city 1 in time 15,
 * but that would be slower than riding it to city 2 in time 6 and then using the blazingly fast horse in city 2 to get to city 1 in just 10/1000 extra time.
 *
