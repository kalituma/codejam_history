 ...
 * It starts off with a beam with a strength of 1, and it will run a program that is a series of instructions,
 * which will be executed one at a time, in left to right order. Each instruction is of one of the following two types:
 ...
 * T test cases follow. Each consists of one line containing an integer D
 * and a string P: the maximum total damage our shield can withstand, and the robot's program.
 */

/*
 * Output
 * For each test case, output one line containing Case #x: y,
 * where x is the test case number (starting from 1) and y is either the minimum number of hacks needed to accomplish the goal, or IMPOSSIBLE if it is not possible.
 */
 ...
 * Sample Case #4 uses the program described in the problem statement. The statement demonstrates one way to reduce the total damage to 5 using two hacks.
 * It is not possible to reduce the damage to 6 or less by using only one hack; remember that the President can only swap adjacent instructions.
