# https://github.com/kamyu104/GoogleCodeJam-2018/blob/master/World%20Finals/the-cartesian-job.py
...
def no_overlapped_interval(interval, s, e):
    ...
    if compare_relative_tan(interval[0], (1, 0)) < 0 and \
            compare_relative_tan((1, 0), interval[1]) < 0:
        # overlapped around theta = 0
        if compare_tan(s, no_overlapped_interval[0]) < 0:
            s = no_overlapped_interval[0]
    elif compare_relative_tan(interval[0], (-1, 0)) < 0 and \
            compare_relative_tan((-1, 0), interval[1]) < 0:
